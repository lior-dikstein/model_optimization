...
def run_operation(n: BaseNode,
    ...
    if mode == ModelBuilderMode.QUANTIZED and n.activation_quantization_cfg is not None and n.activation_quantization_cfg.enable_activation_quantization:
...
class PytorchModelBuilder(torch.nn.Module):
    ...
    def forward(self,
        ...
        Returns:
            A torch Tensor which is the output of the model logic.
        ...
        if self.append2output:
            for n in self.append2output:
                output += node_to_output_tensors_dict.get(n)
        else:
